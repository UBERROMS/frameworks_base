...
public class BatteryMeterDrawable extends Drawable implements
    ...
    public BatteryMeterDrawable(Context context, Handler handler, int frameColor, int style, boolean isBatteryTile) {
        mContext = context;
        mHandler = handler;
        mStyle = style;
    ...
    private void handleBoltVisibility() {
        ...
        if (d instanceof BitmapDrawable) {
            ...
            bd.getPaint().set(!mPluggedIn || (mPluggedIn && mShowPercent == 1 && (!mForceChargeBatteryText || mForceChargeBatteryText))
                                            || (mPluggedIn && mShowPercent == 2)
                                            || (mPluggedIn && mShowPercent == 0  && mForceChargeBatteryText)
                                            ? mClearPaint : mTextAndBoltPaint);
            if (mIsBatteryTile) {
                mBoltDrawable.setTint(getBoltColor());
            }
        } else {
            d.setAlpha(!mPluggedIn || (mPluggedIn && mShowPercent == 1 && (!mForceChargeBatteryText || mForceChargeBatteryText))
                                    || (mPluggedIn && mShowPercent == 2)
                                    || (mPluggedIn && mShowPercent == 0  && mForceChargeBatteryText) ? 0 : 255);
        }
    }
